...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  poseROS.header.frame_id = string("world_enu"); //改
  ...
  velROS.header.frame_id = string("world_enu"); //改
  ...
  if (cov_pos)
  {
    ...
    covROS.header.frame_id = string("world_enu"); //改
  ...
  if (cov_vel)
  {
    ...
    covVelROS.header.frame_id = string("world_enu"); //改
  ...
  if ((t - pt).toSec() > 0.5)
  {
    trajROS.header.frame_id = string("world_enu"); //改
  ...
  // Sensor availability
  sensorROS.header.frame_id = string("world_enu"); //改
  ...
  if (1)
  {
    ...
    broadcaster->sendTransform(tf::StampedTransform(transform,   msg->header.stamp, string("world_enu"),  string("/base")));   //改   
...
int main(int argc, char** argv)
{
  ...
  n.param("frame_id",   _frame_id, string("world_enu") );   //改 
